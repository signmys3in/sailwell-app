...
interface AppContextType {
  ...
  dispenseDrug: (drugId: string, quantity: number, patientName: string, diseases: string[]) => void;
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const dispenseDrug = (drugId: string, quantity: number, patientName: string, diseases: string[]) => {
    setDrugStock((prevStock) =>
      prevStock.map((drug) =>
    ...
    setDispenseLog((prevLog) => [
        ...prevLog,
        {
            ...
            quantity: quantity,
            timestamp: new Date(),
            diagnosis: diseases.join(', ') || 'AI-assisted diagnosis',
            diseases: diseases || []
        }
    ]);
  };
